	...
	def wrapped(*args, **kwargs):
		...
		try:
			try:
				...
				# print(['Entered log for ', func_name])
				...
				if not hasattr(devs,'texec'):
					...
					print(['Added texec attribute to ', func_name])
				else:
...
def getKolmogorovComplexity(m):
	"""
	Computes the Kolmogorov complexity of a given bit of Python code
	When used with DEVS models, it is intended for use on the model transition functions
	Author: J. Scott Thompson
	Email: j.scott.thompson.12@gmail.com
	"""
	c = 0.0
	cls = m.__class__
	block = m.getBlockModel()
	...
	### Get class of model
	if block.python_path.endswith('.py'):
		funcs = [getattr(cls, fct) for fct in block.activity.values() if hasattr(cls, fct)]

		for func in funcs:
			c += kc.getKolmogorovComplexityFunc(func)
...
class ActivityData:
	...
	def getStaticData(self):
		...
		if not self._static_data:
			...
			### static informations
			colLabels = [_('Model'), _('Id'), _('MCC'), _('KC'), _('length'),
			...
			try:
				with Pool() as p:
					...
					kc_list = list(p.imap(getKolmogorovComplexity, model_list))
				...
				kc_list = list(map(getKolmogorovComplexity, model_list))
			...
			data = list(map(lambda a,b,c,d,e,f,g,h,i,j,k,m : [a,b,c,d,e,f,g,h,i,j,k,m[0],m[1]], model_name_list, model_id_list, mcCabe_list, kc_list, h, N, vol, calculated_lenght, loc, lloc, sloc, timeIt_list))

			worst_case, best_case = zip(*timeIt_list)
			
			try:
				with Pool() as p:
					l = list(p.imap(sum, (mcCabe_list, kc_list, h, N, vol, calculated_lenght, loc, lloc, sloc, worst_case, best_case)))

				data.append(['Total', '-']+l)
			except:
				data.append(['Total', '-', sum(mcCabe_list), sum(kc_list), sum(h), sum(N), sum(vol), sum(calculated_lenght), sum(loc), sum(lloc), sum(sloc), sum(worst_case), sum(best_case)])
